...
floating point and classifying an image in 9.1~ms on a desktop CPU with no GPU
present. Training converged in nine epochs and stopped early, with the best
validation $F_1$ of 0.9775 recorded at epoch four. The result carries a caveat that we treat as a
central finding rather than a footnote: a frozen ImageNet feature extractor,
...
The test partition holds 1121 images, 744 labeled \texttt{REAL} and 377 labeled
\texttt{FAKE}, giving a class ratio of 1.97:1. Table~\ref{tab:splits} projects
the remaining partitions from that count under the 70/15/15 ratio.

\begin{table}[t]
\caption{Partition sizes. The test row is measured from the evaluation run; the
training and validation rows are projected from it under the 70/15/15 split and
carry per-denomination rounding error.}
\label{tab:splits}
\centering
\small
\begin{tabular}{@{}lrrrl@{}}
...
\textbf{Total} & \textbf{Source} \\
\midrule
Train      & $\approx$3472 & $\approx$1759 & $\approx$5231 & projected \\
Validation & $\approx$744  & $\approx$377  & $\approx$1121 & projected \\
Test       & 744           & 377           & 1121          & measured \\
\midrule
\textbf{Corpus} & $\approx$4960 & $\approx$2513 & $\approx$7473 & projected \\
\bottomrule
\end{tabular}
\end{table}

The audit and split scripts write per-denomination inventories to
\texttt{reports/}, which the repository excludes along with \texttt{data/}.
Because the split runs per denomination with independent rounding at each of the
fourteen leaf directories, the projected rows in Table~\ref{tab:splits} deviate
from the true counts by a few images per cell.
\tofill{replace the projected rows with measured counts from
reports/dataset\_report.csv before submission}
...
30-epoch horizon. Batch size is 32. Gradients clip to unit $\ell_2$ norm.
Automatic mixed precision~\cite{micikevicius2018mixed} runs with loss scaling,
unscaling before the clip so the threshold applies to true gradient magnitudes.
...
The pipeline gates mixed precision on \texttt{torch.cuda.is\_available()}, so
training requires a CUDA device to reach the reported throughput and falls back
to full-precision CPU execution elsewhere. Training ran on
\tofill{GPU model and total wall-clock time}. Inference carries no such
dependency, and the latency figures in Section~\ref{sec:cost} come from a CPU-only
build.
...
A 9.1~ms median leaves room on slower hardware: even a tenfold penalty on a
mid-range phone SoC keeps a single verification under 100~ms. INT8
post-training quantization~\cite{jacob2018quantization} would cut the footprint
to roughly 3.6~MB, and we have measured neither its latency nor its effect on
accuracy.
...
\item \textbf{Desktop-only latency.} We timed inference on a desktop CPU. No
measurement exists for a phone SoC, which is the deployment target the design
assumes.
\end{itemize}
...
model that trains in nine epochs and answers in 9.1~ms on a CPU. The engineering
result is solid, and the pipeline is reproducible end to end.
